...
Zz.ListFactory = function(ref, blueprint) {
	...
	} else if (typeof blueprint[1] === 'object'
			&& !Object.isArray(blueprint[1])) {
 ...
 */
Zz.List.prototype.push = function(write){
...
Zz.List.prototype.get = function(key){
	...
	var m = this.modelFactory.create(key);

	// TODO implemen method load ... it's better then directly call ref.once here
	
	// traverse the blueprint, compare the DB object and populate the new model with values
	// TODO for now forget about nested objects
...
/**
 * loads the DB content and instantiate the Model
 * with ModelFactory provided
 *
 * @param {string} DB key
 * @returns the instantiated object
 *
 * TODO
 * do we use promise or event ready?
 */
Zz.List.prototype.get = function(key){
	var obj = this.modelFactory();
	this.ref
		.child(key)
