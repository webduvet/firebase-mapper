...
Zz.List.prototype.push = function(){
	...
	var arg,
		...
		obj = this.factory.create(record.key());

	if (arg = arguments[0]) {
		arg.__id = item.key();
		id.set(arg);
	}
	// TODO use factory provided....
	record.set( obj , function(){
...
Zz.ModelFactory.prototype.create = function(url){
	if (url && url.lenght > 1) {
		var ref_ = this.ref.child(url);
	}
	return new Zz.Model(ref_ || this.ref, this.blueprint);
};
