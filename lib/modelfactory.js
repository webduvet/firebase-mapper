...
Fm.ModelFactory = function(ref, blueprint, Model){
	...
	if (Model && !(Model instanceof Fm.Model)) throw new Error("Model if provided nmust inherit from Fm.Model");
};
...
Fm.ModelFactory.prototype.create = function(key){
	...
	if (!key) {
		return new Model(this.ref, this.blueprint);
	} else{
	   	return new Model(ref.child(key), this.blueprint);
	}
};
