 ...
 * @access public
 *
 * @param {Firebase} instance of Firebase reference, at runtime
 * @param {Object} blueprint represents the data structure, static
 * @param {class Fm.Model} either simple Fm Model, static
 */
Fm.ModelFactory = function(ref, blueprint, Model){

	if (typeof ref === 'string') {
		// convert string to Firebase reference
		ref = new Fm.Db(ref);
	} else if ( /^https/.test(ref.toString()) ) {
		this.ref = ref;
	}

	this.blueprint = blueprint;
	this.Model = Model || Fm.Model;
	if (Model && !(Model.prototype instanceof Fm.Basic)) throw new Error("Model if provided must inherit from Fm.Model");
};
