...
Fm.Model = function(ref, blueprint){
	...
	for(var key in blueprint){
		if (Array.isArray(blueprint[key])){
			...
			if(type === 'model') {
				// TODO this will create anonymous factory. how do we know it is fm object not a plin object?
				this[key] = (new Fm.ModelFactory(bp, ref)).create();
			} else if (type ==='list' ) {
				// the moment we learn it contains list (not the best practice)
				// we need to prevent this model from direct write operation
				this[key] = (new Fm.ListFactory(ref, bp)).create();
				this[write] = null;
			}
	...
	/**
	 * TODO what if we assign a plain object to property? how do we do watchLocal?
	 * perhaps we just come up with solution how to deep watch plain objects
	 * or do all properties as getter setter and if value is an object then iterate it and instantiate model for it on every level
	 */
	function make(bp) {
		for ( var key in bp ) {
			if ( Array.isArray(bp[key] && bp[key][0] === 'model' ) {
